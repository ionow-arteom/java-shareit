...
public class RequestControllerTest {
    ...
    void addRequest() throws Exception {
        when(itemRequestService.add(any(ItemRequestDto.class), anyLong())).thenReturn(firstItemRequestDto);
        ...
        verify(itemRequestService, times(1)).add(firstItemRequestDto, 1L);
    }

    @Test
    void getRequests() throws Exception {
        when(itemRequestService.get(anyLong())).thenReturn(List.of(firstItemRequestDto, secondItemRequestDto));
    ...
    void getAllRequests() throws Exception {
        when(itemRequestService.getAll(anyLong(), anyInt(), anyInt())).thenReturn(List.of(firstItemRequestDto, secondItemRequestDto));
        mvc.perform(get("/requests/all")
                ...
                .andExpect(status().isOk())
                .andExpect(content().json(mapper.writeValueAsString(List.of(firstItemRequestDto, secondItemRequestDto))));
        verify(itemRequestService, times(1)).getAll(1L, 0, 10);
    }

    @Test
    void getRequestById() throws Exception {
        when(itemRequestService.getById(anyLong(), anyLong())).thenReturn(firstItemRequestDto);
