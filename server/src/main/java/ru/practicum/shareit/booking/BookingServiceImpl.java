...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingOutDto approve(long userId, long bookingId, Boolean approved) {
        ...
        if (approved) {
            if (booking.getStatus().equals(Status.APPROVED)) {
                throw new ValidationException("Неверный статус запроса на обновление состояния");
            }
    ...
    public BookingOutDto getById(long userId, long bookingId) {
        ...
        if (booking.getBooker().getId() == userId || booking.getItem().getOwner().getId() == userId) {
            ...
            throw new NotFoundException(User.class, "Для получения информации о бронировании или владельце предмета {} " + userId + " можно использовать id владельца предмета");
        }
    }
